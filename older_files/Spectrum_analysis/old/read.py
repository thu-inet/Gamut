import old.energy_calibration_unuseful as energy_calibration_unuseful

def read(filepath,start=0,end=-1):
    '''
    ...
    try:
        if filepath[-4:] in ['.spe','.Spe','.chn','.Chn']:
            while True:
                line=filelines[indl]
                if '$ENER_FIT:' in line:
            ...
            while True:
                try:
                    erg=energy_calibration_unuseful.energy_calibration(chn,slope=slope,increment=increment)
                    line=filelines[indl]
                    line=line.split()
            ...
            while True:
                try:
                    ...
                    break
        return list_erg[start:end],list_eff[start:end]
    except:
